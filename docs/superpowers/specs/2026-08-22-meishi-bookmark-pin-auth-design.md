...
- 2026-08-23: さらに改善提案を反映。(1)README.mdが「紙の名刺の登録・写真添付機能は一時停止中」という古い記述のままだったのを、復活済み(`paper-card.html`、一覧・編集・検索込み)の実態に合わせて更新。バックアップ機能・レート制限への言及も追加。(2)`paper-card.html`で紙の名刺の写真をタップすると、従来は新規タブでリンクを開くだけだったのを、ページ内で拡大表示するライトボックス(`#lightbox-modal`)に変更（背景クリックまたは閉じるボタンで閉じる）。実機で表示・閉じる挙動を確認済み
- 2026-08-23: ユーザー指示により、「紙の名刺」というユーザー向け表示名を「S-NEXUA 撮影名刺」に変更。対象は`paper-card.html`の`<title>`/`<h1>`/モーダル見出し/一覧の空状態メッセージ/登録失敗時のエラーメッセージ、`index.html`のリンク文言、`about.html`・`README.md`の説明文中の言及。コード内コメント・テストの説明文字列・過去の仕様書(2026-08-21design.md等)の記述は内部識別子として変更対象外とした
- 2026-08-23: CI改善。(1)`.github/workflows/deploy.yml`にtestジョブを追加し、deployジョブが`needs: test`で依存するように変更（従来はテスト59件が一度もCI上で実行されず、壊れていても検知されなかった）。(2)`upload-pages-artifact`の対象を`path: '.'`（リポジトリ全体）から、rsyncで`docs/`・`gas-backend/`を除いた`dist/`に変更（開発用の設計メモや`.clasp.json`のscriptId等を公開対象から除外）。(3)追加直後、GitHub Actions環境(Node.js 20)で`node --test 'src/**/*.test.js'`のglobパターンが解決できず「Could not find」で失敗する問題が発覚（ローカルのNode.js 22では動いていたため未検知だった）。`package.json`の`test`スクリプトを`node --test $(find src gas-backend -name '*.test.js')`に変更し、シェル・Node.jsバージョンに依存しない方式にして解決。本番環境でdocs/・gas-backend/が404（非公開）、src/が200（公開）になっていることを実機確認済み
- 2026-08-23: GAS backend(`gas-backend/Code.gs`)の改善2件。(1)`saveBookmarks`がbookmarksの中身を一切検証していなかったため、合言葉さえ分かれば巨大なダミーデータを送りつけられ、スプレッドシートの1セル文字数上限(約5万字)を超過→以後そのユーザーが保存不能になるリスクがあった。件数上限(500件)・JSON化後の文字数上限(45000字)を追加し、超過時は`TOO_MANY_BOOKMARKS`/`PAYLOAD_TOO_LARGE`エラーで拒否するように修正。(2)`findUserRow`が合言葉照合のたびに発行済み合言葉を全行スキャンしており、発行数が増えるほど遅くなる懸念があったため、`CacheService`で合言葉→行番号をキャッシュ（`issueCode`側でも発行直後に先回りしてキャッシュ投入）。キャッシュヒット時も1セル読み取りでシート側とのズレが無いか検証する。claspで既存デプロイを更新（version @7）、本番環境で件数超過・サイズ超過が正しく拒否され、既存データが壊れないことを実機確認済み
- 2026-08-23: [重要な訂正] 前回追加した`checkRateLimit`(合言葉ごとのレート制限)は、合言葉の総当たり攻撃には無力であることが判明。攻撃者は毎回違う合言葉を試すため、キー(`rl_<code>`)が毎回変わり制限に引っかからない。合言葉に依存しないグローバルなキーで「合言葉が見つからなかった(CODE_INVALID)」回数を数え、1分間に50回を超えたら一時的に全体を`RATE_LIMITED`にする`checkGlobalInvalidCodeLimit()`/`recordInvalidCode()`を追加。正規の合言葉への通常アクセスはこのカウンタを消費しない。`checkRateLimit`側のコメントも、これが総当たり対策ではないことを明記するよう修正。claspで既存デプロイを更新（version @8）、本番で正規アクセス・CODE_INVALIDの両方が正しく動作することを実機確認済み（グローバル制限の実発火は本番の他ユーザーに影響するため、ユニットテスト側で検証）
- 2026-08-23: [重要な再訂正] コードレビューにより、直前の`checkGlobalInvalidCodeLimit`自体が新たなDoSベクターになっていたことが判明。このチェックはauth無しで誰でも到達でき、存在しない合言葉を1分間に50回POSTするだけで、その後1分間、正規ユーザー全員のget_bookmarks/save_bookmarksが巻き込まれてブロックされてしまう。総当たり対策を撤回し、「合言葉は32^8通りありGAS自体の実行クォータが実質的な歯止めになっている」という元の前提に戻した。合わせて、レビューで見つかったもう1件のバグ（`saveBookmarks`がbookmarksが配列でない場合に黙って空配列に読み替え、既存データをエラーも出さず上書き消去していた）を修正し、`INVALID_PAYLOAD`エラーで拒否するようにした。claspで既存デプロイを更新（version @9）、本番で両方の修正を実機確認済み
- 2026-08-23: 同レビューで見つかった軽微な保守性の指摘2件も対応。`findUserRow`が、キャッシュ検証時と全行スキャン時で`getSheet()`を別々に呼んでいたのを1回に統合。また、キャッシュされた行がシート側の実際の行とズレていた（削除・並び替え等）場合に古いキャッシュを消さず放置し、以降の呼び出しのたびに同じ無駄な検証読み取りを繰り返していたのを、ズレ検知時に`cache.remove()`するように修正。claspで既存デプロイを更新（version @10）、本番で疎通確認済み
- 2026-08-23: 追加の改善提案を反映。バックエンドのエラーコード(TOO_MANY_BOOKMARKS/PAYLOAD_TOO_LARGE/RATE_LIMITED/CODE_INVALID)に対し「次に何をすればいいか」を追記する`describeApiError()`を`src/pocketApi.js`に追加し、`index.html`/`paper-card.html`の全エラーハンドリング箇所(11箇所)をこれ経由に統一。あわせて、`index.html`の`describeGoogleSyncError`が401/403以外の場合に元のエラーメッセージを捨てて固定文言に置き換えていたため、`describeApiError`が作った具体的な案内がユーザーに届かない問題を修正（`err.message`があればそれを優先）。本番で「持っている合言葉を入力する」に存在しない合言葉を入れ、「合言葉が見つかりません 合言葉に入力ミスがないか確認してください。」が表示されることを実機確認済み
